 ...
 *
 */
void BBB_Driver_I2C_Read_Write_Outputs_wrapper(const boolean_T *inI2C_WR_Enable,
...
int iI2C_Read_Register(char *cRegister_Address, char *cRegister_Value)
{
    ...
    if (iDebug_Level >=Debug_Level_2)  { printf("I2C_ReadWrite Msg: I2C Bus File Location: %s\n",sBBB_I2C_BUS1_FILE); }
    ...
    {
        // Error Handling
        if (iDebug_Level >=Debug_Level_1)  { printf("I2C_ReadWrite Error: Failed to Open I2C Bus - Check Device/Device Tree\n"); }
        // Set Error Number
    ...
    // Ouput Program Flow
    if (iDebug_Level >=Debug_Level_3) {printf("I2C_ReadWrite Msg: Program Flow State 2: Connect To I2C Bus\nError No: %i Error Level: %i\n",iError_No, iError_Level);}
    ...
    if (iError_No == Error_None)
    {
        ...
        {
            if (iDebug_Level >=Debug_Level_1) {printf("I2C_ReadWrite Error: Failed to Communicate With Slave Device: %#04x\n",cDevice_I2C_Addr);}
            // Set Error Number
    ...
    // Ouput Program Flow
    if (iDebug_Level >=Debug_Level_3) {printf("I2C_ReadWrite Msg: Program Flow State 3: Connect to I2C Device\nError No: %i Error Level: %i\n",iError_No, iError_Level);}
    ...
    if (iError_No == Error_None)
    {
        ...
        {
            // Error Handling
            if (iDebug_Level >=Debug_Level_1) { printf("I2C_ReadWrite Error: Failed to Write/Communicate With Device and/or Set Buffer Read Range\n");}
            // Set Error Number
        ...
        {
            if (iDebug_Level >=Debug_Level_1) { printf("I2C_ReadWrite Msg: Buffer Range Set\n");}
        }
    }

    // Ouput Program Flow
    if (iDebug_Level >=Debug_Level_3) {printf("I2C_ReadWrite Msg: Program Flow State 4: Write to Device Registry\nError No: %i Error Level: %i\n",iError_No, iError_Level);}
    ...
    if (iError_No == Error_None)
    {
        ...
        if (iI2C_BytesRead<0)
        {
            ...
            if (iDebug_Level >=Debug_Level_1) { printf("I2C_ReadWrite Error: Failed to Read Device Register: %#04x Bytes Read: %i\n",cRegister_Address,iI2C_BytesRead );}
            // Set Error Number
        ...
        {
            // Registry Data Read!
            if (iDebug_Level >=Debug_Level_1) { printf("I2C_ReadWrite Msg: Device Register Read. Bytes Read: %i\n",iI2C_BytesRead );}
...
int iI2C_Write_Register(char cRegister_Address, char cRegister_Value)
{
    ...
    if (iError_No == Error_None)
    {
        ...
        if (iIOCTL_BBB_I2C_Handle<0)
        {
            if (iDebug_Level >=Debug_Level_1) {printf("I2C_ReadWrite Error: Failed to Communicate With Slave Device\n");}
            // Set Error Number
            iError_No = Error_No_I2CInit;
            // Set Error Level
            iError_Level = Error_Level_Critical;
    ...
    if (iError_No == Error_None)
    {
        ...
        {
            // Error Handling
            if (iDebug_Level >=Debug_Level_1) { printf("I2C_ReadWrite Error: Failed to Communicate With Device and/or Write To Device Register\n");}
            // Set Error Number
        ...
        {
            // Power Mode Set
            if (iDebug_Level >=Debug_Level_1){printf("I2C_ReadWrite Msg: Data Written to Gyro Register!\n");}
        }
    }
    // Close File
    close(iFILE_BBB_I2C_Handle);
...
// Check Parameters
if (iDebug_Level >=Debug_Level_1){printf("I2C_ReadWrite Msg: Input Parameter Data: I2CBus: %#04x I2CAddress: %#04x\n",prmI2C_BusNo[0],prmI2C_DeviceAddress[0]);}

// Ouput Program Flow
if (iDebug_Level >=Debug_Level_3) {printf("I2C_ReadWrite Msg: Program Flow State 1: Parameters Initialized \nError No: %i Error Level: %i\n",iError_No, iError_Level);}

// Record Device Address
cDevice_I2C_Addr = prmI2C_DeviceAddress[0]; //0x68
cDevice_I2C_RegAddr[0] = prmI2C_RWRegister[0];
cDevice_I2C_WriteValue=inI2C_W_Value[0];

// Read Parameter Data -Block Function Mode: Read or Write
switch(prmI2C_Mode[0])
{
    case  1: //Read Mode:
        ...
        {
            // Error Handling
            if (iDebug_Level >=Debug_Level_1){printf("I2CRead Error: Failed to Read Registery %#04x From Device\n",prmI2C_RWRegister[0]);}
        }
        else
        {
            // Error Handling
            if (iDebug_Level >=Debug_Level_1){printf("I2CRead Msg: Successfully Read Device Register %#04x. Value Read: %#04x\n",prmI2C_RWRegister[0],cI2C_Buffer[0]);}
        }          
        // Break       
        break;
    case  2: //Write Mode       
    ...
    default:
        ...
        // Error Handling
        if (iDebug_Level >=Debug_Level_1) { printf("I2C_ReadWrite Block Error: Read/Write Mode Selection Not Recognized\n");}
        // Set Error Number
        iError_No = Error_No_I2CBuffValidate;
        // Set Error Level
        iError_Level = Error_No_ParamOutofRange;
        // Return Otuput Error Level
        // return -1;     
        break;
}
...
// Ouput Program Flow
if (iDebug_Level >=Debug_Level_3) {printf("I2C_ReadWrite Msg: Program Flow State 6: Loop Step Complete\nError No: %i Error Level: %i\n",iError_No, iError_Level);}
...
if ((iError_No!=0)&&(iError_Level==Error_Level_Critical))
{
    ...
    if (iDebug_Level >=Debug_Level_0) {printf("I2C_ReadWrite Msg: Critical Error Detected; Simulation Stopping! \nError No: %i Error Level: %i\n",iError_No, iError_Level);}
    
    // Output Error Message:
    if (iDebug_Level >=Debug_Level_0) {printf("I2C_ReadWrite Error Details: %s",sI2CReadWrite_ErrMessage(iError_No));}
...
else
{
    ...
    outI2C_WR_Data[0]=cI2C_Buffer[0];
...
/*
  ...
  *
  */
void BBB_Driver_I2C_Read_Write_Update_wrapper(const boolean_T *inI2C_WR_Enable,
                          const uint8_T *inI2C_W_Value,
                          const boolean_T *inI2C_WR_Trigger,
...
if (xD[0]==0)
{
    ...
    if (iDebug_Level >=Debug_Level_1){printf("I2C_ReadWrite Msg: Program Flow State: Init First Scan Complete\n");}
}
